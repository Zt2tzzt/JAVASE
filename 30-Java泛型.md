...
Java 泛型，是伪泛型，只在编译时期起作用。

- 事实上，集合中存储的，仍然是 Object 类型的对象；
- 只是在数据添加时，会检测数据类型是否符合泛型的限定。
...
- 泛型中不能写基本数据类型；
- 指定泛型的具体类型后，转嘀数据时，可以传入该类型或者其子类型。
- 如果不写泛型，类型默认是 Object。

Java 泛型，可以写在方法、类、接口上，它们分别称为泛型方法、泛型类、泛型接口。
...
- 方案一：在方法所在类的类名处，定义泛型；该泛型在类中都可以使用。
...
- 方式一：实现类给出具体的泛型。
- 方式二：实现类是泛型类，延续了接口泛型，创建泛型类对象时，再确定泛型。
...
方式二理解：定义一个 `MyArrayList03` 泛型类，实现 `List` 接口，并延续 `List` 接口的泛型。
...
- `? extends E`，表示 E 和它的所有子类类型。
- `? super E`，表示 E 和它的所有父类类型。
