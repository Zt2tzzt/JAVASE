...
- 如果是无序的数组，先排序，再进行二分查找，只能用于证明元素在数组中是否存在，查找到的索引没有意义；
- 因为排序可能打乱元素顺序，查找到的元素索引不准确。
...
分块查找原则 2：快数数量一般等于数字的个数开根号，比如：16 个数字一般分为 4 快左右。
...
使用分块查找，查找一组完全没有规律的数据，如下图所示，每个分块相对于其它分块，应该有一个都立的区间值。
...
创建一个容器，它的容量为根号 10，然后分别把不同范围内的数据，作为“块”存入容器中，如下图所示：
...
斐波那契查找（Fibonacci Search）是一种基于斐波那契数列的查找算法，适用于已排序的数组。它利用斐波那契数列来确定查找范围，从而逐步缩小搜索区间。
...
冒泡排序，核心思路是响铃的数据，两两比较，小的放前面，大的放后面。

1. 第一轮比较，会将容器中的最大值，放在最右边；
2. 第二轮比较，会将容器中的次大值，放在最大值的左边；
3. 依此类推……
...
核心思路是：从容器的 0 索引开始，与后面的元素一一比较，小的放前面，大的放后面。
...
递归，只需要用少量的程序，就可以描述出解题过程所需要的多次重复计算。用于把一个复杂的问你，层层转化为一个与原问题相似的规模较小的问题来求解。
...
9. 把基准数左边看做一个序列，把基准数右边看做一个序列，按照上述的规则递归排序。
