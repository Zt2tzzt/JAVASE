...
- 八个比特位分为一组，就是一个字节，它可以存储（2^8^ = 256）个数字。

字节，是计算机中最小的存储单元。

计算机在存储一个英文字母时，会使用 ASCLL 字符集，进行编码，编码后的结果，仅需要一个字节存储即可。
...
对于汉字来说，也需要一张类似于 ASCLL 的码表，将所有汉字字符，与唯一的一个数字对应。
...
- 措施一：不用字节流，读取纯文本文件；
- 措施二：编码、解码都使用同一种字符集，并按照同一个编码规则进行。

> 使用字节流拷贝文件，不会出现乱码；
>
> 只要打开拷贝文件的编辑器，使用的是原文件的编码规则即可。
...
Java `String` 类解码的方法如下，是利用 `String` 类的构造方法实现的：
...
public class CharsetDemo01 {
    public static void main(String[] args) throws UnsupportedEncodingException {
        ...
        // 解码
        String msg1 = new String(bytes1); // 默认解码方式：IDEA 使用 UTF-8 解码规则
        System.out.println(msg1); // wee最棒

        String msg2 = new String(bytes2);
        System.out.println(msg2); // wee���

        String msg3 = new String(bytes1, "GBK"); // 指定 GBK 的解码方式
