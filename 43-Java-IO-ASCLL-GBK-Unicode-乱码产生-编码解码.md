...
回顾：计算机存储规则：在计算机中，任意数据都是以二进制的形式来存储的。
...
计算机在存储一个英文字母时，会使用 ASCLL 字符集，进行编码，只需要一个字节即可。
...
Unicode 字符集，同样也完全兼容 ASCLL 字符集。它有很多的编码规则，比如：
...
编码的过程，就是蒋要存储的数据，按照特定字符集的编码规则，转为字节，存储在硬盘中；

所以，使用 Java `String` 类编码，返回值是一个字节数组。常用的的方法如下：
