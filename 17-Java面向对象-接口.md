...
在 Java 中，接口与类之间，是实现关系；类实现接口，可以多实现，也可以在继承类的同时多实现接口。
...
定义一个抽象类，`Person` 类，但其中没有抽象方法；

- 定义为抽象类的原因，主要是不想让外界直接创建该类的实例对象，因为抽象类不能创建实例对象。
...
JDK8 及以后，接口中的**默认方法**，要用 `default` 关键字修饰：
...
- 特点一：JDK8 及以后，接口中的默认方法，不是抽象方法，所以不强制子类实现（重写）。如果被实现（重写），子类实现（重写）的方法上要去掉 `default` 关键字。
...
此时，就必须在子类 `InterImpl1` 中实现（重写）两接口中，同名的 `defaultMethod` 方法。
...
JDK9 及以后，接口里可以存在 `private` 修饰的私有方法，是为了
...
JDK9 及以后，接口中的私有方法，要用 `private` 关键字修饰；
...
> **设计模式（Design Pattern）**，指的是一套被反复使用，多数人知晓，经过分门别类的，代码设计经验的总结。
>
...
1. 编写中间类 `XxxAdapter`，实现接口。在类中**空实现**接口中的所有抽象方法；
