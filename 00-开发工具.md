...
project（项目、工程）最 IDEA 的最外层层级。它的下一层是 module（模块）。
...
为了更好的管理代码，我们会把代码分别放在以上两个模块中存放。不同的模块，可以存放项目中不同的业务功能代码。
...
2. 为 package（包）命名，习惯以公司域名的反写，加上 package（包）名来命名；
   ...
   - 这个步骤，本质上是在 `src` 目录下，新建了一个 `com` 目录，在它下面又新建了一个 `kkcf` 目录，在它下面又新建了一个 `demo01` 目录；
...
新建项目；打开 IDEA 开发工具，创建一个 project（项目、工程）
