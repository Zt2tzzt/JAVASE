...
- 成员内部类 `Inner1` 是公开的，可以直接在外界创建其对象。
...
外界调用静态内部类里的非静态成员（方法）的步骤：1.先创建对象；2.再用对象调用。
...
局部内部类，可以直接访问外部类的成员，也可以直接访问方法内的局部变量
...
- 实现 / 继承：`{}` 中的类才是字面意义上的匿名内部类，它实现或继承了 `类名或接口名`。
- 方法重写：实现了接口，就要实现里面的抽象方法；继承了抽象类，就要重写里面的抽象方法。
- 创建对象：`new` 操作符，创建了这个匿名类的对象。`()` 表示用空参构造创建的对象。

> 匿名内部类，无需开发者手动命名；JDK 在编译时，会给匿名内部类起名，比如 `Xxx$1`，`Xxx$2`。
>
> - `Xxx` 一般是外部类名。
...
在测试类 `Test` 中，创建匿名内部类，实现 `Swim` 接口，继承 `Animal` 抽象类：
...
当一个类，只需要使用一次，可以省略定义类的步骤，直接使用匿名内部类，定义类并创建它的对象；
