...
如果想要在电脑的任意位置，打开 QQ，那么要将 qq.exe 记录在环境变量中。

如果想要在 CMD 的任意目录下，都可以启动某一个软件（比如 QQ），那么就可以把这个软件的路径，配置到环境变量中的 `PATH` 里面。

在启动软件的时候，操作系统会先在当前路径下找，如果在当前录课没有，再到环境变量的路径 `PATH` 中去找。如果都找不到就提示无法启动。
...
Java 代码不能再操作系统中直接运。要将它翻译为操作系统能够识别的机器码。

> 新版本的 jdk（比如 JDK17），在无需配置环境变量的情况下，也可以使用 `javac` 命令、`Java` 命令。
>
> - `javac` 是 JDK 提供的一个编译工具。
> - `java` 是 JDK 提供的运行字节码文件的工具。
>
> JDK17 在安装完成后，会在 PATH 环境变量中，配置 `C:\Program Files\Common Files\Oracle\Java\javapath`
>
...
使用 jdk 的 bin 目录下 javac 命令，去编译这个文件：

在代码所在目录，带卡 cmd 命令窗口，输入命令：
...
> 新版本的 JDK，安装后会默认在 `PATH` 下配置 `C:\Program Files\Common Files\Oracle\Java\javapath`；
>
...
- 现在环境变量中，创建一个 `JAVA_HOME` 的环境变量，值设为 JDK 的安装目录，即：`D:\Devtools\Java\jdk-17`；

### 2.PATH 环境变量中引用 JAVA_HOME

然后再 `PATH` 环境变量中，新增一条 `%JAVA_HOME%\bin\`，即可；

以后用到的开发工具，如果引用了 Java 相关工具，就回去这个目录下查找。

## 六、Java 语言的发展

1990 年，Sun（Stanford University Network）公司，James Gosling 领导了 Green 计划，为下一代家电（电视机、微波炉、电话）编写一个通用的控制系统。

- 原本打算使用 C++ 来编写项目，但是发现 C++ 有很多的不方便的地方；
- 于是 James Gosling 创建了 Oak（橡树）语言。

1992 年，Green 计划已完成了新平台的部分功能，包括 Green 操作系统；Oak 的程序设计语言，类库等等。同年 11 月，Sun 公司成立了一个新公司，名为 FirstPerson。但该公司一度因经营不善，差点凉凉，Oak 语言也差点结束。
...
- 然而，桌面应用常用的复杂动画，Java 加载的速度有可能很慢，在桌面应用领域，最合适的是 C 和 C++。
...
1. 编程：Java 代码在 .java 文件中编写；C 代码在 .c 文件中编写；python 代码在 .py 文件中编写。
2. 编译：机器只认识 0、1 组成的机器码，编译就是将代码文件，转为机器码的过程。
3. 运行：让机器执行编译后的指令。
...
- **解释性语言**；
  ...
  - Java，C 语言被编译后，都会产生一个编译文件；但 python 是直接使用 .py 文件来运行代码的。也就是说 python 必须要将源代码交给运行环境来运行，而不像 Java、C 语言一样，仅需要不编译好的编译文件交给机器运行。
...
Java 不会直接在操作系统上运行，而是运行在虚拟机（即 JVM）中的。
...
- **Java 虚拟机（JVM）**，我们知道，Java 虚拟机（JVM）是真正运行 Java 字节码文件的地方。

- **核心类库**。Java 已经写好的东西，可以直接使用。

  ```java
  System.out.println("HelloWorld");
  ```

  - `System`、`out`、`println` 都是核心类库中定义好的。
...
上方的 **JVM** 、**核心类库**两部分内容，和 **Java 运行工具**（比如 `Java`）组成了 **JRE（Java Runtime Environment）**，表示 Java 运行环境。JRE 仅仅用来执行编译好的字节码文件即可。
