...
- 定义了一个 int 类型的数组。
  - boolean。double 类型的数据，是不能存到这个数组中的，
...
数组初始化的方式分为两种，分别是静态初始化，动态初始化。

### 1.数组静态初始化

数组的静态初始化，可分为“完整格式”和“简写格式”，一般习惯使用间歇格式。
...
Java 数组动态初始化，指的是在数组初始化时，只指定数组的长度，由系统为数组分配初始值。
...
要避免这个异常，就要明确数组的最小索引是 `0`，最大索引是 `arr.length - 1`。
...
JVM 为了更好的利用这块空间，将这块空间分为了五部分，它们分别是：
...
JDK 7 以前，方法区、堆空间，是连在一起的，在真是的物理内存当中，也是一块连续的空间。
...
下方代码，用于演示当两个数组类型的变量，指向同一块顿内存空间时的场景。
