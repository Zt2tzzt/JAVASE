...
- 表达式，表示将要匹配的值，只能匹配 byte，short，int，char 类型的值，JDK5 以后可以是枚举类型的值，JDK7 以后可以是 String 类型的值。
...
- 然而，当 for 循环中，控制循环体的变量，写在 for 循环体外面，那么效果就于 while 循环相同了，如下方所示
...
- 回文数是指正序（从左到右）和倒序（从右到左）读都是一样的整数；例如 121 是回文数，123 不是回文数。
