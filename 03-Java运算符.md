...
- 自增运算符 `++`；
  - 变量的前面有 `++`，称为前置自增运算符，比如：`++a`；
  - 变量的后面有 `++` ，称为后置自增运算符。 比如：`a++`。
- 自减运算符 `--`。
  - 变量的前面有 `--`，称为前置自增运算符，比如：`--a`；
  - 变量的后面有 `--`，称为后置自增运算符。 比如：`a--`。

前置自增/自减运算符，表示”先加/减后用“。

后置自增/自减运算符，表示”先用后加/减“。

自增自减运算符在处理 `byte`、`short` 和 `char` 类型时，同样会先隐式转换（自动类型提升）位 `int` 类型，再计算。
...
- 结论：对 `char` 类型变量使用自增或自减运算符时，Java 会将它们提升为 `int` 类型进行运算，然后再转换回 `char` 类型。
...
| != | 判断左边跟右边是否不相等，如果成立就是 `tru`，如果不成立就是 f alse |
...
Java 中的三元运算符，又叫三元表达式，或者问好冒号表达式。
...
- 正数计算时，反码就是原码；
- 负数计算时，反码在原码的基础上，符号位不变，数值取反（即 `0` 变 `1`，`1` 变 `0`）。

以数字 `56` 举例：

- `56` 的原码是：符号位 `0`（表示正数），加数字 `011 1000`，即 `0011 1000`；
- `-56` 的原码是：符号位 `1`（表示负数），加数字 `011 1000`，即 `1011 1000`。
- `-56` 的反码是：符号位 `1`（与原码保持一致），加数字 `100 0111`（在原码的基础上取反），即 `1100 0111`。
...
如果有负数参与的计算，最终得到的结果大于等于 `0`，那么使用反码计算，也会出现问题。
...
- 然而 `-5 + 6 = 1`，与 `0` 不符，它们之间差生了 `1` 的误差，

产生 `1` 误差的原因，是因为 `0` 的反码有两种形态，即 `0000 0000` 和 `1111 1111`。
...
- byte 类型的 a 变量是一个正数 `10`，原、反、补码的表现形式一致，即 `0000 1010`。
- int 类型的 b 变量，是变量 a 隐式转换后的值，原、反、补码的表现形式为 `0000 0000 0000 0000 0000 0000 0000 1010`
...
| >>>    | 无符号右移 | 向右移动，高位补 0（不管原来是正数还是负数，都补 0）                                    |
