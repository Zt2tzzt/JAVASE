...
如果桌子上有多杯咖啡（队列中有多个元素），那么：

- 厨师会将做好的咖啡，放入（`put`）到队列中，直到队列放满后，会等待，也称为**阻塞**。
...
- `put` 方法底层**使用了锁**，不用自行再定义锁，否则会造成锁的嵌套。
...
- `take` 方法底层**使用了锁**，不用再自行定义锁，否则会造成锁的嵌套。
...
1. `NEW`（新建），表示线程对象创建。
2. `RUNNABLE`（就绪），表示 `start` 方法调用；
3. `BLOCKED`（阻塞）， 表示无法获得锁对象。
4. `WAITING`（等待），表示 `wait` 方法调用。
5. `TIMED_WAITING`（计时等待），表示 `sleep` 方法调用。
6. `TERMINATIED`（结束），表示全部代码运行完毕。
